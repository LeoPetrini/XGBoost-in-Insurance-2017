...
output_cas <- scoring_gam_xgb_offset(db = dbtweedie, xvarCat = xvarCat, xvarCont = xvarCont,
                                     ...
                                     weight_tweedie = "Exposure" , xgb_CS = xgb_CS_casdata ,
                                     xgb_NB = xgb_NB_casdata, xgb_tweedie = xgb_tweedie_casdata , nfolds=10)


df1 = output_cas[[1]]
df2 = output_cas[[2]]
write.csv(df1, "cas_df_time2.csv")
write.csv(df2, "cas_df_preds2.csv")
...
output_noncas <- scoring_gam_xgb_offset(db = db4ModelFull, xvarCat = xvarCat, xvarCont = xvarCont,
                                        ...
                                        weight_tweedie = "exposure" , xgb_CS = xgb_CS,
                                        xgb_NB = xgb_NB, xgb_tweedie = xgb_tweedie , nfolds=10)

df1_noncas = output_noncas[[1]]
df2_noncas = output_noncas[[2]]
write.csv(df1_noncas, "noncas_df_time2.csv")
write.csv(df2_noncas, "noncas_df_preds2.csv")

# Final GAM Metrics 
# RMSE:  2530.085 LOGLOSS:  0.3781875 GINI : 0.348239 
# Final XGB Metrics 
# RMSE:  989.6314 LOGLOSS:  0.3864413 GINI : 0.4413764 GINI_TWEEDIE:  0.3766415
